...
# save a clinic - params <clinic_id> <clinic_name> <username> <password> <key_path> <key_pass>
saveClinic() {
    echo ''
...
# customize the obibconnector.properties for a clinic - params <clinic_id> <password>
customizeProperties() {
    echo ''
...
registerClinic() {
    ...
    echo ' notes:'
    echo '  - fields marked with (*) are information provided or generated by CDX.'
    echo '  - fields marked with (+) are information related to the exported CDX Vendor Certificate.'
    echo '  - fields marked with (!) are new information required to generate the OBIB Connector Certificate.'
        ...
        while true; do
            ...
            case $opts in
                [Cc]* ) if saveClinic "$clinic_id" "$clinic_name" "$username" "$password" "$key_path" "$key_pass"; then
                            ...
                            customizeProperties "$clinic_id" "$password"
                        fi
                        break 2;;
                [Rr]* ) break;;
                [Qq]* ) echo 'bye.'; exit;;
